...
Label 16 tubes, four each of: E (oil emulsion), T (transfer), and L (liposomes). Pipette the mastermix below into tube E1, then aliquot 30 µL per E tube into the remaining 3 E tubes (E2–E4) for encapsulation. Reserve 16 µL (2 × 8 µL) for plate reader kinetics and 10 µL for osmolarity measurement.
...
5. Centrifuge the T tube at 9000 g (8970 rpm for tabletop centrifuges, r = 10 cm) for 10 min at room temperature to pellet the liposomes.
   a. Arrange tubes so that the hinge side faces out; take note of where the pellet will form (hinge side) after centrifugation.
6. Extract the liposomes from the T tube:
   a. Remove the oil layer and lipid debris from the top (supernatant) of the T tube by gently pipetting with a **200 µL** pipette set to 100 µL. Begin at the top of the mixture and aspirate from the solution surface as the liquid level descends in the tube. Dispose of supernatant in designated container or an empty microcentrifuge tube.
   b. Once the majority of supernatant is removed, extract liposomes by pipetting 50 µL of pellet and outer solution from tube T and transfer to the respective liposome tube L. Do not transfer the entire solution; avoid transferring the top of the solution which may contain a residual oil layer.
...
5. Centrifuge the T tube at 9000 g (8970 rpm for tabletop centrifuges, r = 10 cm) for 10 min at room temperature to pellet the liposomes.
   a. Arrange tubes so that the hinge side faces out; take note of where the pellet will form (hinge side) after centrifugation.
6. Extract the liposomes from the T tube:
   a. Remove the oil layer and lipid debris from the top (supernatant) of the T tube by gently pipetting with a **1000 µL** pipette set to 150 µL. Begin at the top of the mixture and aspirate from the solution surface as the liquid level descends in the tube. Dispose of supernatant in designated container or an empty microcentrifuge tube. Dislodge and resuspend the pellet by gently pipette-mixing near the pellet. Pipette 50 µL of pellet and outer solution from tube T and transfer to the respective liposome tube L. Do not transfer the entire solution; avoid transferring the top of the solution which may contain a residual oil layer.
...
:width: 90%
Mean green intensity of encapsulated PURE reaction over 4 h. Green represents fluorescence from the modified extraction method; red represents fluorescence of the standard extraction method.
:::
...
:width: 90%
Fog plot of mean intensity per liposome over the full time course — modified method. Each individual dot represents an encapsulated PURE reaction and the intensity at which it is fluorescing.
:::
...
:width: 90%
Fog plot of mean intensity per liposome over the full time course — standard method. Each individual dot represents an encapsulated PURE reaction and the intensity at which it is fluorescing.
:::
...
:::{table} Mean green fluorescence value by method and timepoint.
:label: tbl-results
:align: center
| Method | T = 0 | T = 3 h | T = 5 h |
| --- | --- | --- | --- |
...
Samples were processed with ImageJ software. Each image was selected for highest density within the well, then taken at beginning, middle, and end of time course.
:::
...
Comparisons of modified and standard methods were performed using aliquots of the same PURE reaction set up with two true replicates per method. Figures 1, 2, and Table 1 show the density and intensity of the modified method to be greater than the standard method; however, reproducibility between replicates is lacking for the modified method. Both methods show similar trends of increasing intensity over time (Figure 2). However, the overall number of liposomes is seemingly much greater in the standard method (Figure 4). Overall, the greater intensity of sfGFP fluorescence and liposome density seen in Figure 1 points to the modified method being a suitable alternative to the standard method; however, issues stemming from reproducibility are still in question.
